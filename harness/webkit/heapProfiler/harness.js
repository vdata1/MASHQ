...
var CheapHeapSnapshotNode = class CheapHeapSnapshotNode {
  constructor(snapshot, nodeIndex) {
    ...
    this.className = snapshot.classNameFromTableIndex(
      nodes[nodeIndex + nodeClassNameOffset]
    );
    ...
    if (firstEdgeIndex !== nodeNoEdgeValue) {
      for (
        ...
        i < snapshot.edges.length;
        i += edgeFieldCount
      ) {
        if (snapshot.edges[i + edgeFromIdOffset] !== this.id) break;
...
var CheapHeapSnapshotEdge = class CheapHeapSnapshotEdge {
  constructor(snapshot, edgeIndex) {
    ...
    this.type = snapshot.edgeTypeFromTableIndex(
      edges[edgeIndex + edgeTypeOffset]
    );

    if (this.type === "Property" || this.type === "Variable")
      this.data = snapshot.edgeNameFromTableIndex(
        edges[edgeIndex + edgeDataOffset]
      );
    else this.data = edges[edgeIndex + edgeDataOffset];
  }
...
var CheapHeapSnapshot = class CheapHeapSnapshot {
  constructor(json) {
    ...
    for (let i = 0; i < edges.length; ) {
      ...
      assert(
        ...
        "Edge list should be ordered by from node identifier"
      );
      if (fromIdentifier !== lastNodeIdentifier) {
        let nodeIndex = this._nodeIdentifierToIndex.get(fromIdentifier);
        assert(
          this._nodes[nodeIndex + nodeIdOffset] === fromIdentifier,
          "Node lookup failed"
        );
...
var HeapSnapshotNode = class HeapSnapshotNode {
...
var HeapSnapshotEdge = class HeapSnapshotEdge {
...
var HeapSnapshot = class HeapSnapshot {
  constructor(json) {
    ...
    for (let i = 0; i < edges.length; ) {
      ...
      if (type === "Property" || type === "Variable") data = edgeNames[data];
    ...
    assert(
      this.rootNode.outgoingEdges.length > 0,
      "<root> should have children"
    );
    assert(
      this.rootNode.incomingEdges.length === 0,
      "<root> should not have back references"
    );
  }
  ...
  nodesWithClassName(className) {
    ...
    for (let node of this.nodes) {
      if (node.className === className) result.push(node);
    }
    return result;
  }
};
...
function followPath(node, path) {
  ...
  for (let component of path) {
    ...
    if (component.edge)
      ...
      edges = current.outgoingEdges.filter(
        (e) => e.to.className === component.node
      );
    assert(
      edges.length === 1,
      "Ambiguous or bad path component: " + JSON.stringify(component)
    );
    current = edges[0].to;
  }
  return current;
