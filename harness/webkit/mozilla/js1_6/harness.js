...
function printStatus(msg) {
  ...
  for (var i = 0; i < lines.length; i++) writeLineToLog(STATUS + lines[i]);
}
...
function reportCompare(expected, actual, description) {
  ...
  if (output != "") {
    if (typeof description != "undefined") reportFailure(description);
...
function exitFunc(funcName) {
  ...
  if (funcName) {
    ...
    if (lastFunc != funcName)
      reportFailure(
        "Test driver failure, expected to exit function '" +
          funcName +
          "' but '" +
          lastFunc +
          "' came off " +
...
function BigO(data) {
  ...
  function LinearRegression(data) {
    ...
    var Syx2 =
      (SUM_Ydiff2 - Math.pow(SUM_XdiffYdiff / SUM_Xdiff2, 2)) / (n - 2);
    var r2 =
      Math.pow(n * SUM_XY - SUM_X * SUM_Y, 2) /
...
function JavaScriptOptions() {
  ...
  if (typeof options == "function") {
    ...
    var prefService = preferences.getService(
    ...
    try {
      this.orig.strict = this.strict = pref.getBoolPref(
        "javascript.options.strict"
      );
    } catch (e) {}

    try {
      this.orig.werror = this.werror = pref.getBoolPref(
...
JavaScriptOptions.prototype.setOption = function (optionName, optionValue) {
  if (typeof options == "function") {
    // shell
