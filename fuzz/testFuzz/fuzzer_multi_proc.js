...
/*function applyDelete(ast, removeNode){
            ...
            if(Array.isArray(parent.body)){
                if(parent.body.some(
                    (child)=>
                        child == removeNode && //
                        child._fuzzFlag && 
                        child._fuzzFlag.operation == "delete"
                
                )){
                    console.log("Deleting node...");
                    return estraverse.VisitorOption.Remove; 
                    //this.remove();
                    //parent.body.push(removeNode);
                }
            } else if(node.parent && Array.isArray(parent.body.body)){
                if(parent.body.body.some(
                    (child)=>
                        child == removeNode && //
                        child._fuzzFlag && 
                        child._fuzzFlag.operation == "delete"
                    ...
                    //this.remove();
                    //parent.body.body.push(removeNode);
                }
            }
        }
    });
...
function applyDelete(ast, removeNode) {
    estraverse.replace(ast, {
        leave(node, parent) {
            // avoid deleting the whole AST 
            if (!parent || parent.type === 'Program') return;
            
            if (Array.isArray(parent.body)) {
                if (parent.body.includes(removeNode)) {
                    if (node === parent) {
                        console.log("deleting...")
                        //console.log("Deleting parent node...");
                        return estraverse.VisitorOption.Remove;
                    }
                }
            }

            else if (parent.body && Array.isArray(parent.body.body)) {
                if (parent.body.body.includes(removeNode)) {
                    if (node === parent) {
                        console.log("deleting...")
                        //console.log("Deleting parent node from nested body...");
                        return estraverse.VisitorOption.Remove;
                    }
                }
            }
        ...
        leave(node, parent) {   
            ...
            if (node._fuzzFlag) {
                ...
                if(operation === 'transfer'){
                        ...
                        //console.log("fuzzNodeIndex: ", fuzzNodeIndex, "parent: ", parent);
                    ...
                    try{
                        ...
                        if(targetNode && targetNode.body && targetNode.body.body){
                            targetNode.body.body.unshift(...injectedSnippet.body);
                            addedSnippets.push(node._fuzzFlag.snippetAST[runtime]);
                        } else if (targetNode && targetNode.body) {
...
/*
function injectSnippet(ast1, ast2, counter = 1){
    let nodes = [];
    
    //if(counter == 4){
    //    console.log("Tries limit reached... "); 
    //    return ast1; 
    //}
    
    // Traverse AST1 and collect candidate nodes for injection
    estraverse.traverse(ast1, {
        enter: (node) => {
            if(Array.isArray(node.body)){
                nodes.push(node);  // Collect all injectable nodes 
            }
            
        }
    });

    // Pick a random node from AST1 for injection
    ...
    if(nodes.length > 0){
        ...
        estraverse.traverse(ast1, {
            enter: (node) => {
                if(node == randomNode){
                    console.log("*****", ast2);
                    node.body = [...node.body, ...ast2.body];  
                }
                
            }
        });  
    }else{
        ast1.body = [...ast1.body, ...ast2.body]; 
    }
    // Determine if the node is a block where we can insert statements
...
function chooseAndMarkModification(ast, snippets, additionalTestAST, round){//, operation=null) {
    ...
    const weights = [0.20, 0.20, 0.60]//, 0.2]; //15,15,70
...
function applyModificationsAcrossRuntimes(tests, snippets, round, outputDir) {
    ...
    let newASTs = []; 
    console.log("tests Copy Starts");
    console.log("length: ", tests.length);
    let testsCopy = cloneAST(tests); //JSON.parse(stringify(tests));
    console.log("tests Copy Ends")
    //global.gc();
    //console.log(tests);
    tests.forEach(test => {
        if(test.AST != null){
            ...
            } while (testIndex === randIndix && counter <= 20);
              
            //console.log("Check RandIndex: ", randIndix == testIndex); 
            
            //After Modification 
            const additionalTestAST = testsCopy[randIndix].AST; 
            //End Modification 
            ...
            for(let i=0; i<3; i++){ //6
                ...
                let clonedAST = modAST; //cloneAST(modAST);
                global.gc();
                ...
                runtimes.forEach(runtime => {
                    ...
                    let modifiedAST = performRoundWithFlags(clonedAST, runtime); //, availableSnippets[runtime], modification, locations);
...
function fuzz(testsDir, snippetsDir, rounds, outputDir) {
    ...
    const snippets = snippetsDir; //readSnippetFiles(snippetsDir);
    let newTests = []; 
    ...
    for (let round = 1; round <= rounds; round++) {
        ...
        if (!fs.existsSync(roundDir)) fs.mkdirSync(roundDir,  { recursive: true });
